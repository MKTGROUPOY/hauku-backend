...
function normalizeProduct(node) {
  ...
  const ikaRaw  = parseJson(meta['koiran_ika']);
  const kokoRaw = parseJson(meta['koiran_koko']);
  // Normalisoi: "Kaikille ikäluokille" → "Kaikki", "Kaikille kokoluokille" → "Kaikki"
  const ika  = ikaRaw.map(v  => v.startsWith('Kaikki') ? 'Kaikki' : v);
  const koko = kokoRaw.map(v => v.startsWith('Kaikki') ? 'Kaikki' : v);
