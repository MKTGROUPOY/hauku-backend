...
export function buildDirectProductResponse(products, filters) {
  ...
  display.slice(0, maxShow).forEach(p => {
    response += `\n**${p.nimi}**\n`;
    // Raakaruoille: näytä onko täysravinto vai täydennysravinto (olennaista omistajalle)
    if ((p.ruokatyyppi || '').toLowerCase().startsWith('raaka') && p.ruoanTyyppi) {
      const tyypit = String(p.ruoanTyyppi).replace(/\\n/g, ', ').split(/[\n,]/).map(s => s.trim()).filter(Boolean);
      response += `Tyyppi: Raakaruoka — ${tyypit.join(', ')}\n`;
    }
