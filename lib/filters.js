...
const ALLERGEN_INGREDIENT_PATTERNS = {
  ...
  'lohi':       ['lohi', 'lohiöljy', 'lohijauho', 'lohifilee'],
  'lohiöljy':   ['lohiöljy'],
...
function applyCommonFilters(products, filters, includeAllergen = true) {
  return products.filter(p => {
    ...
    // Kauppasuodatus — KAIKISSA poluissa
    if (filters.store) {
      let hasStore = false;
...
export function extractFilters(history) {
  ...
  for (const pr of PROTEINS) {
    ...
    if (pi >= 0) {
      ...
      if (WT.some(t => ctx.includes(t)) && !want.includes(pr)) want.push(pr);
    }
  }
  ...
  return { excl: Object.keys(excl), want, brand: null, age, size, specialDiets, wantLowFat, hasRedFlag, store };
}
...
// buildProductContext säilytetään tulevaa käyttöä varten (dead code poistettu)
export function buildProductContext() { return ''; }
