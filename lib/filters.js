...
export function extractFilters(history) {
  ...
  let store = null;
  // Kauppa tunnistetaan koko keskusteluhistoriasta — jos kauppa mainittu missä tahansa viestissä
  const allUserTxtLow = norm(userMsgs.map(m => m.content).join(' ')).toLowerCase();
  if (/peten|petenkoira|peten koiratarvike/.test(allUserTxtLow)) store = 'petenkoiratarvike';
  else if (/haukkula|koiratarvike haukkula/.test(allUserTxtLow)) store = 'haukkula';
  else if (/zooplus/.test(allUserTxtLow)) store = 'zooplus';
...
export function filterProducts(products, filters) {
  ...
  // Fallback: poista kokorajoitus jos ei tuloksia
...
export function buildProductContext(products, filters) {
  ...
  products.slice(0, maxP).forEach(p => {
    ...
    if (p.v?.length)  lines.push(`Vapaa (allergeenitieto): ${p.v.join(', ')}`);
    ...
    // Näytä kauppa linkin SIJAINNIN perusteella (1=Peten, 2=Haukkula, 3=Zooplus)
    if (p.l)  lines.push(`Ostolinkki (Peten Koiratarvike): ${p.l}`);
    if (p.l2) lines.push(`Ostolinkki (Koiratarvike Haukkula): ${p.l2}`);
    if (p.l3) lines.push(`Ostolinkki (Zooplus): ${p.l3}`);
  });
