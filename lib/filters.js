...
export function filterProducts(products, filters) {
  ...
  function productPassesAllergenCheck(p, exclList) {
    ...
    for (const excl of exclList) {
      ...
      if (vapaa.length > 0) {
        // Vapaa-lista on eksplisiittinen: tuotteen PITÄÄ sisältää merkintä "ei <allergeeni>"
        if (!vapaa.includes(exclLow)) return false;
      } else if (ainesosat.length > 0) {
        // Tarkista ainesosat pattern-listalla
        const patterns = allergenPatterns[exclLow] || [exclLow];
        if (patterns.some(pat => ainesosat.includes(pat))) return false;
      } else {
        // EI vapaa-listaa EI ainesosatietoa → hylkää allergeenitilanteessa (turvallisuus ensin)
        return false;
      }
    }
    return true;
...
export function buildProductContext(products, filters) {
  ...
  lines.push(`EHDOTON SÄÄNTÖ: Suosittele VAIN alla lueteltuja tuotteita. Älä mainitse, keksi tai viittaa mihinkään muuhun tuotteeseen tai brändiin.`);
  ...
  products.slice(0, maxP).forEach(p => {
    ...
    if (p.kp && p.l)  lines.push(`Ostolinkki (${p.kp}): ${p.l}`);
    if (p.kp2 && p.l2) lines.push(`Ostolinkki 2 (${p.kp2}): ${p.l2}`);
  });
