...
export function extractFilters(history) {
  ...
  // Kauppasuodatus laukeaa VAIN kun käyttäjä pyytää tuotteita tietystä kaupasta
  // Pelkkä kaupan mainitseminen ei riitä
  const STORE_TRIGGERS = ['löytyykö', 'onko', 'haen', 'etsin', 'tilaan', 'ostan', 'kaupasta', 'kaupalta', 'kaupalla', 'sivuilta', 'sivuilla'];
  const hasStoreTrigger = STORE_TRIGGERS.some(t => userTxtLow.includes(t));
  if (hasStoreTrigger) {
