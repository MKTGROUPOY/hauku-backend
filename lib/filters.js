...
function productPassesAllergenCheck(p, exclList) {
  ...
  const ainesosat  = (p.a || '').replace(/<[^>]*>/g, ' ').toLowerCase(); // poista HTML-tagit
  const proteiinit = (p.p || []).map(x => x.toLowerCase());

  for (const excl of exclList) {
    const exclLow = excl.toLowerCase();
    const patterns = ALLERGEN_INGREDIENT_PATTERNS[exclLow] || [exclLow];

    // 1. Tarkista proteiinit-kenttä ENSIN — vapaa-merkintä ei voi ohittaa tätä
    //    "Proteiinit: Kana" on ehdoton este vaikka tuote olisi merkitty "vapaa kanasta"
    if (proteiinit.some(pr => patterns.some(pat => pr.includes(pat)))) return false;

    // 2. Eksplisiittinen vapaa-merkintä → allergeeni ok tässä tuotteessa
...
function applyCommonFilters(products, filters, includeAllergen = true) {
  return products.filter(p => {
    ...
    if (filters.store) {
        const allLinks = [p.l, p.l2, p.l3].filter(Boolean);
        const hasStore = allLinks.some(url => isValidStoreUrl(url, filters.store));
...
export function extractFilters(history) {
  ...
  else if (/senior|seniori|vanha |7v|8v|9v|10v|11v|12v|\b[7-9] vuo|\b1[0-9] vuo/.test(latestMsg || allTxt)) age = 'Senior';
  else if (/aikuinen|adult/.test(latestMsg || allTxt)) age = 'Aikuinen';
  // Tunnista ikä vuosista: "3 vuotias" = Aikuinen, "0,5 vuotias" / "8kk" = Pentu
  if (!age) {
    const ageMatch = (latestMsg || allTxt).match(/\b([0-9]+)\s*vuo/i);
    if (ageMatch) {
      const yrs = parseInt(ageMatch[1]);
      if (yrs <= 1) age = 'Pentu';
      else if (yrs >= 7) age = 'Senior';
      else age = 'Aikuinen';
    }
    const kkMatch = (latestMsg || allTxt).match(/\b([0-9]+)\s*kk\b/i);
    if (kkMatch && !age) {
      const months = parseInt(kkMatch[1]);
      if (months <= 12) age = 'Pentu';
    }
  }
  ...
  // Kauppa — tarkista viimeisin viesti ensin, sitten koko historia
  // Näin kauppavalinta pysyy muistissa koko session ajan
  let store = null;
  ...
  if (!store) {
    // Koko historia — kauppavalinta säilyy sessioissa
    const allStoreTxt = norm(userMsgs.map(m => m.content).join(' ')).toLowerCase();
    if (/peten/.test(allStoreTxt)) store = 'petenkoiratarvike';
    else if (/haukkula/.test(allStoreTxt)) store = 'haukkula';
    else if (/zooplus/.test(allStoreTxt)) store = 'zooplus';
  }
...
export function filterProducts(products, filters) {
  ...
  if (res.length === 0 && filters.size) {
    res = products.filter(p => {
      ...
      if (!p.l && !p.l2 && !p.l3) return false;
      if (filters.store) {
        // Tarkista kaikki linkkikentät — Haukkula-URL voi olla missä tahansa kentässä
        const allLinks = [p.l, p.l2, p.l3].filter(Boolean);
        const hasStore = allLinks.some(url => isValidStoreUrl(url, filters.store));
        if (!hasStore) return false;
      }
...
export function buildDirectProductResponse(products, filters) {
  ...
  let response = ''; // täytetään turvaverkon jälkeen oikealla luvulla

  // Turvaverkko: näytä vain tuotteet joilla on toimiva linkki valitulle kaupalle
  const displayProducts = filters?.store
    ? products.filter(p => {
        const allLinks = [p.l, p.l2, p.l3].filter(url => url && url.trim().length > 5);
        return allLinks.some(url => isValidStoreUrl(url, filters.store));
      })
    : products;

  // Päivitä myös lukumäärä oikeaksi
  const displayCount = filters?.store ? displayProducts.length : products.length;
  response = `Löysin ${displayCount} sopivaa tuotetta${storeSuffix}:\n`;

  displayProducts.slice(0, maxP).forEach(p => {
    response += `\n**${p.n}**\n`;
    ...
    if (filters?.store) {
      const allLinks = [
        { url: p.l,  name: 'Peten Koiratarvike' },
        { url: p.l2, name: 'Koiratarvike Haukkula' },
        { url: p.l3, name: 'Zooplus' },
      ];
      const match = allLinks.find(l => l.url && isValidStoreUrl(l.url, filters.store));
      if (match) response += `Ostolinkki (${match.name}): ${match.url}\n`;
    } else {
...
export function buildProductContext(products, filters) {
  ...
  products.slice(0, maxP).forEach((p, idx) => {
    ...
    // Ainesosat ja ravintoarvot jokaiselle tuotteelle — ei trunkkausta
    // Geminin täytyy nähdä kaikki ainesosat voidakseen vastata oikein
    if (p.a) lines.push(`Ainesosat: ${p.a}`);
    if (p.rv) {
      let rvClean = p.rv;
      try { const parsed = JSON.parse(rvClean); rvClean = Array.isArray(parsed) ? parsed[0] : String(parsed); } catch {}
      rvClean = String(rvClean).replace(/^\["|"\]$|^\[|\]$/g, '').trim();
      if (rvClean) lines.push(`Ravintoarvot: ${rvClean}`);
    }
