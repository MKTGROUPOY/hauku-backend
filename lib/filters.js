...
function ageMatches(productIka, filterAge) {
  ...
  if (filterAge === 'Pentu')   return /pentu|junior|kaikille ikäluokille/.test(joined);
  if (filterAge === 'Aikuinen') return /aikuinen|kaikille ikäluokille/.test(joined);
  if (filterAge === 'Senior')  return /senior|kaikille ikäluokille/.test(joined);
  return true;
}
