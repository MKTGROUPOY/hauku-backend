...
export function filterProducts(products, filters) {
  ...
  const allergenPatterns = {
    'kana': ['kana', 'kanaliha', 'kananliha', 'kananrasva', 'broileri', 'siipikarja', 'siipikarjanliha', 'siipikarjanrasva'],
...
export function buildProductContext(products, filters) {
  ...
  // Rakenna HYVÄKSYTTYJEN tuotteiden lista — Gemini saa VAIN nämä
  const approvedNames = products.slice(0, maxP).map(p => p.n);

  const lines = [];
  lines.push(`\n\n<BACKEND_TUOTEVALINTA>`);
  lines.push(`BACKEND ON VALINNUT ${products.length} TUOTETTA${el}. SINÄ MUOTOILET VAIN NÄMÄ ${maxP} TUOTETTA.`);
  lines.push(`EHDOTON SÄÄNTÖ: Käytä VAIN alla olevia tuotteita. ÄLÄ lisää, korvaa tai mainitse muita tuotteita.`);
  lines.push(`HYVÄKSYTYT TUOTTEET: ${approvedNames.join(' | ')}`);
  lines.push(`---`);

  products.slice(0, maxP).forEach((p, idx) => {
    const tt = p.tt ? `[${p.tt}] ` : '';
    lines.push(`TUOTE ${idx + 1}: ${tt}${p.n} | Merkki: ${p.m}`);
    ...
    // Täydet tiedot vain ensimmäiselle (exactProduct)
    if (idx === 0 || maxP <= 2) {
  ...
  lines.push(`MUISTUTUS: Käytä VAIN yllä olevia ${maxP} tuotetta. Älä mainitse muita.`);
  lines.push(`</BACKEND_TUOTEVALINTA>`);

  return lines.join('\n');
}

// Suora backendivastaus ilman Geminiä — käytetään fallbackina
export function buildDirectProductResponse(products, filters) {
  ...
  products.slice(0, maxP).forEach(p => {
    ...
    // Rasvapitoisuus voi olla JSON-string ["arvo"] tai tavallinen teksti
    let rl = p.rl || '';
    ...
    // Näytä vain pyydetyn kaupan linkki jos kauppasuodatus on aktiivinen
    if (filters?.store === 'petenkoiratarvike' && p.l) {
      ...
      // Ei kauppasuodatusta — näytä kaikki saatavilla olevat linkit
