...
function productPassesAllergenCheck(p, exclList) {
  ...
  // Siipikarja (poultry) = kana for allergen purposes
  const exclHasKana = exclList.some(e => ['kana','kananrasva','siipikarja','siipikarjanrasva'].includes(e.toLowerCase()));
  if (exclHasKana) {
    const proteins = (p.p || []).map(x => x.toLowerCase());
    const ingredients = (p.a || '').toLowerCase();
    if (proteins.some(pr => pr === 'siipikarja' || pr === 'siipikarjanrasva' || pr === 'siipikarjaproteiini')) return false;
    if (ingredients.includes('siipikarjan') || ingredients.includes('siipikarjaproteiini')) return false;
  }
...
export function filterProducts(products, filters) {
  ...
  const filtered = products.filter(p => {
    ...
    // Ikä — myös tuotenimi tarkistetaan jos Shopify-data puuttuu
    if (filters.age) {
      const nameLow = (p.n || '').toLowerCase();
      const isPuppyName = /puppy|junior|pentu/.test(nameLow);
      const isSeniorName = /senior|mature|7\+|adult 5\+|adult 7\+/.test(nameLow);
      // Jos nimi kertoo selkeästi iän, käytä sitä
      if (isPuppyName && filters.age !== 'Pentu') return false;
      if (isSeniorName && filters.age === 'Pentu') return false;
      // Käytä myös Shopify-kenttää
      const ika = p.i || [];
      if (ika.length > 0) {
        const ok = ika.some(x => x === filters.age || x.startsWith('Kaikki'));
        if (!ok && !isPuppyName && !isSeniorName) return false;
      }
    }
  ...
  // Sorttaa: parhaiten matchaavat ensin
  filtered.sort((a, b) => {
    let scoreA = 0, scoreB = 0;
    if (filters.age) {
      if ((a.i || []).includes(filters.age)) scoreA += 3;
      if ((b.i || []).includes(filters.age)) scoreB += 3;
    }
    if (filters.size) {
      if ((a.k || []).some(x => x === filters.size)) scoreA += 2;
      if ((b.k || []).some(x => x === filters.size)) scoreB += 2;
    }
...
export function extractFilters(messages) {
  ...
  // Kerää sanat joita saa SYÖDÄ ("saa syödä kananrasvaa" → kananrasva on OK)
  ...
  if (hasTrigger || /ei saa|ei voi|vapaa|allergi|herkk/.test(allTxt)) {
    for (const [kw, allergens] of Object.entries(ALLERGEN_MAP)) {
      // Ohita jos käyttäjä sanoi "saa syödä X"
      if (okIngredients.has(kw)) continue;
      if (allTxt.includes(kw)) {
        // Varmista että konteksti on "ei X" eikä "haluaa/saa syödä X:ää"
        const wantPattern = new RegExp(`(haluaa|tykkää|mieluusti|suosittele|sopii|saa syödä|voi syödä)\s{0,20}${kw}`, 'i');
