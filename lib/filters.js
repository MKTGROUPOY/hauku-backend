      ...
      for (const excl of filters.excl) {
        ...
        if (vapaa.length > 0) {
          ...
          const allergenPatterns = {
            ...
            'herne': ['herne'],
            'kalaöljy': ['kalaöljy', 'kala öljy'],
            'lohiöljy': ['lohiöljy', 'lohi öljy'],
...
export function buildProductContext(products, filters) {
  ...
  return lines.join('\n');
}n\\n<tuotteet_tietokannasta>\\nVALIKOIMASSA ON TASAN ${products.length} TUOTETTA${el}. ÄLÄ SANO MUUTA LUKUA. Tässä ne kaikki:`]/ lib/filters.js — Suodatuslogiikka suomen kielen taivutuksineen
// Tämä on sama logiikka kuin hauku-widget.html:n JS, mutta Node.js-muodossa
