...
export function extractFilters(history) {
  ...
  // ── Want/excl konflikti: EXCL voittaa – poista wantista jos myös excl:ssä ────────────────
  ...
  // ── Kauppasuodatus ───────────────────────────────────────────────────────────
  let store = null;
  if (/peten|pete[nl]|petenkoira/.test(norm(userTxt))) store = 'peten';
  else if (/haukkula|koiratarvike haukkula/.test(norm(userTxt))) store = 'haukkula';
  else if (/zooplus/.test(norm(userTxt))) store = 'zooplus';
...
export function filterProducts(products, filters) {
  ...
  let res = products.filter(p => {
    ...
    if (filters.store) {
      ...
      const hasStore = (p.l && p.l.toLowerCase().includes(storeNorm)) ||
                       (p.l2 && p.l2.toLowerCase().includes(storeNorm)) ||
                       (p.kp && p.kp.toLowerCase().includes(storeNorm)) ||
                       (p.kp2 && p.kp2.toLowerCase().includes(storeNorm));
...
export function buildProductContext(products, filters) {
  ...
  const lines = [`\n\n<tuotteet_tietokannasta>\nLöydetty ${products.length} sopivaa tuotetta${el}. TÄMÄ ON TÄYDELLINEN LISTA – ei muitakaan tuotteita:`];
  ...
  // Brändihaulla näytetään enemmän tuotteita
  const isBrandSearch = products.length > 5 && products.every(p => p._allergenWarning !== undefined || true);
  ...
  lines.push('\n</tuotteet_tietokannasta>');
  lines.push('EHDOTON SÄÄNTÖ: Suosittele VAIN yllä listattuja tuotteita nimeltä. ÄLÄ KOSKAAN mainitse, keksi tai suosittele tuotteita jotka eivät ole tässä listassa – vaikka tietäisit niiden olevan olemassa muualla maailmassa. Jos sopivaa tuotetta ei listalla ole, sano se rehellisesti.');
