...
export function filterProducts(products, filters) {
  ...
  function score(p) {
    let s = p.prio === 1 ? 10 : 0;
    if (filters.age && p.ika.some(x => x.toLowerCase().includes(filters.age.toLowerCase()))) s += 2;
    if (filters.size) {
      const sizeWord = filters.size === 'Keski' ? 'keskikokoinen' : filters.size.toLowerCase();
      if (p.koko.some(x => x.toLowerCase().includes(sizeWord))) s += 1;
    }
    return s;
  }
