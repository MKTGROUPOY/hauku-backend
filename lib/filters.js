...
// Allergeenien partitiivimuodot luettavaa tekstiä varten
const PARTITIIVI = {
  'Kana': 'kanaa', 'Kananrasva': 'kananrasvaa', 'Kalkkuna': 'kalkkunaa',
  'Kalkkunanrasva': 'kalkkunanrasvaa', 'Kananmuna': 'kananmunaa',
  'Nauta': 'nautaa', 'Naudanrasva': 'naudanrasvaa',
  'Lammas': 'lammasta', 'Lampaanrasva': 'lampaanrasvaa',
  'Possu': 'possua', 'Sianrasva': 'sianrasvaa',
  'Kala': 'kalaa', 'Kalaöljy': 'kalaöljyä', 'Lohi': 'lohta',
  'Vehnä': 'vehnää', 'Kaura': 'kauraa', 'Ohra': 'ohraa',
  'Maissi': 'maissia', 'Herne': 'hernettä', 'Soija': 'soijaa',
  'Soijaöljy': 'soijaöljyä', 'Peruna': 'perunaa', 'Riisi': 'riisiä',
  'Ankka': 'ankkaa', 'Ankanrasva': 'ankanrasvaa',
};
export function allergenText(excl) {
  return (excl || []).map(a => PARTITIIVI[a] || a.toLowerCase()).join(', ');
}
...
export function buildDirectProductResponse(products, filters) {
  ...
  // Asiakkaan allergeenit luettavassa muodossa — näytetään joka tuotteen
  // alla VAIN koska suodatus on jo varmistanut asian datasta
  const allergiaRivi = filters?.excl?.length
    ? `✓ Ei sisällä: ${allergenText(filters.excl)}\n`
    : '';
